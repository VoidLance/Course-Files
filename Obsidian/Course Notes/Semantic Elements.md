...
- `<figure> <figcaption>` is used to define a demonstrative or diagrammatical figure, and to caption it
	- Where caption is a property for a standard `<img>` tag, `<figcaption>` is a separate element that is included inside `<figure></figure>` element [[Tags/Tags]].
	- `<img>` [[Tags/Tags]] are also still required inside `<figure>` [[Tags/Tags]] in order to add the relevant image
